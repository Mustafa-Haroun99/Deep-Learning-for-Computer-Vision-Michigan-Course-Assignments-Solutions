...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    actual_start = 10 - (start%10) + start
    if actual_start > stop:
      x = torch.tensor([], dtype=torch.float64)
    else:
      x = torch.arange(actual_start, stop + 1, 10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x.view(2, 3, 4).transpose(0, 1).reshape(3, 8).clone()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    M = x.shape[0]
    mu = x.sum(dim=0)/M
    sigma = ((((x-mu)**2).sum(dim=0))/(M-1))**0.5
    y = ((x-mu)/sigma).clone()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    ls[0] -= 1
    concatenated_tensor = torch.cat(xs, dim=0)
    cumulative_sum = torch.cumsum(concatenated_tensor, dim=0)
    slices = cumulative_sum[torch.cumsum(ls, dim=0)]
    shifted_slices = torch.zeros(slices.shape[0]+1)
    shifted_slices[1:] = slices[:].clone()
    y = ((slices-shifted_slices[:-1])/ls) 
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
