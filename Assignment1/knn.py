...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        dists[i, j] = ((((x_test_flatten[j]-x_train_flatten[i])**2).sum())**0.5).item()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_flatten = x_test.view(num_test, -1)
    for i in range(num_train):
      dists[i] = (((x_test_flatten - x_train_flatten[i])**2).sum(dim=1))**0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flatten = x_train.view(num_train, -1)
    x_test_flatten = x_test.view(num_test, -1)
    x_train_sq = (x_train_flatten**2).sum(dim=1).view(-1, 1)
    x_test_sq = (x_test_flatten**2).sum(dim=1)
    dists = (x_train_sq - 2*torch.mm(x_train_flatten, x_test_flatten.t()) + x_test_sq)**0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    topk = torch.topk(dists, k, dim=0, largest=False).indices
    for i in range(num_test):
      topk[:, i] = y_train[topk[:, i]]
      majority = 0
      for j in range(10):
        class_occurance = (topk[:, i] == j).sum()
        if class_occurance > majority:
          majority = class_occurance
          y_pred[i] = j
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        ...
        x_test = temp_x.pop(i)
        x_train_new = torch.cat(temp_x, dim=0)
        temp_y = y_train_folds.copy()
        y_test = temp_y.pop(i)
        y_train_new = torch.cat(temp_y, dim=0)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies:
      num_folds = len(k_to_accuracies[k])
      current_acc = sum(k_to_accuracies[k])/num_folds
