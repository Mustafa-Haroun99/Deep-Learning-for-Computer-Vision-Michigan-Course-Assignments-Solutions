...
def svm_loss_vectorized(
    ...
    scores[scores < 0] = 0
    scores[idx, y] = 0
    loss += (scores.sum()/num_train) + reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    # Replace "pass" statement with your code
    temp_x = torch.broadcast_to(X, (num_classes, num_train, num_dimensions)).clone()
    temp_x = temp_x.permute(1, 2, 0)
    indices = (scores==0).nonzero()
    temp_x[indices[:, 0].reshape(-1), :, indices[:, 1].reshape(-1)] = 0
    scores[(scores!=0)] = 1
    scores = scores.sum(dim=1).reshape(num_train, 1)
    temp_x[idx, :, y] = -X*scores
    dW += temp_x.sum(dim=0)
...
def test_one_param_set(
    ...
    # Replace "pass" statement with your code
    loss_history = cls.train(data_dict['X_train'], data_dict['y_train'], lr, reg
    , num_iters)
    y_preds = cls.predict(data_dict['X_train'])
    train_acc = ((y_preds == data_dict['y_train']).sum()/(len(y_preds))).item()
    y_preds = cls.predict(data_dict['X_val'])
    val_acc = ((y_preds == data_dict['y_val']).sum()/(len(y_preds))).item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    C = W.shape[1]
    scores = torch.mm(X, W)
    maxs = torch.broadcast_to(scores.max(dim=1)[0].reshape(-1, 1), (N, C))
    scores -= maxs
    scores = torch.exp(scores)
    for i in range(N):
      summ = (scores[i].sum()) + 1e-8
      correct_class_probability = (scores[i, y[i]]/summ) + 1e-8
      loss += -torch.log(correct_class_probability).item()
      for c in range(C):
        derivative = correct_class_probability * (-scores[i, c]/summ)
        if c == y[i]:
          derivative += correct_class_probability
          dW[:, c] += (-1/(correct_class_probability))*(derivative * X[i])
          continue
        dW[:, c] += (-1/(correct_class_probability))*(derivative * X[i])
    loss /= N
    loss += reg * (W**2).sum()
...
def softmax_loss_vectorized(
    ...
    scores = torch.exp(scores)
    idx = torch.arange(N)
    correct_scores = scores[idx, y]
    summ = scores.sum(dim=1) + 1e-8
    correct_scores_probability = (correct_scores/(summ)) + 1e-8
    loss += -(torch.log(correct_scores_probability).sum())
    loss /= N
    loss += reg * (W**2).sum()

    summ = torch.broadcast_to(summ.reshape(-1, 1), (N, C))
    broadcasted_correct_scores_probability = torch.broadcast_to(correct_scores_probability.reshape(-1, 1), (N, C))
    scores /= -summ
    scores *= broadcasted_correct_scores_probability
    scores[idx, y] += correct_scores_probability
    scores /= -broadcasted_correct_scores_probability
    dW += (torch.mm(scores.t(), X)/N).t() + 2*reg*W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
