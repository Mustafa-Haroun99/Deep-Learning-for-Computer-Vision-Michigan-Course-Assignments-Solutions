...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = torch.mm(X, W1) + b1.reshape(1, -1)
    hidden[hidden<0] = 0
    scores = torch.mm(hidden, W2) + b2.reshape(1, -1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    # Replace "pass" statement with your code
    C = W2.shape[1]
    maxs = scores.max(dim=1)[0].reshape(-1, 1)
    s0 = scores - maxs
    s1 = torch.exp(s0)
    idx = torch.arange(N)
    s2 = s1.sum(dim=1) + 1e-8
    s3 = s1[idx, y]
    s4 = (s3/(s2)) + 1e-8
    loss = -(torch.log(s4).sum())
    loss /= N
    ...
    ds4_ds3 = 1/s2
    dl_ds3 = dl_ds4 * ds4_ds3

    ds4_ds2 = -(s3/(s2**2))
    dl_ds2 = dl_ds4 * ds4_ds2

    ds3_ds1 = scores.new_zeros((N, N, C))
    ds3_ds1[idx, idx, y] = 1
    dl_ds1_1 = (dl_ds3.reshape(-1, 1, 1) * ds3_ds1).sum(dim=0)

    ds2_ds1 = scores.new_zeros((N, N, C))
    ds2_ds1[idx, idx, :] = 1
    dl_ds1_2 = (dl_ds2.reshape(-1, 1, 1) * ds2_ds1).sum(dim=0)

    dl_ds1 = dl_ds1_1 + dl_ds1_2
    
    ds1_ds0 = s1
    dl_ds0 = dl_ds1 * ds1_ds0

    ds0_dscores = torch.zeros_like(scores) + 1
    dl_dscores = dl_ds0 *ds0_dscores

    grads["W2"] = (torch.mm(dl_dscores.t(), h1)).t() + 2*reg*W2
    grads["b2"] = dl_dscores.sum(dim=0)

    dl_dh1 = torch.mm(dl_dscores, W2.t())
    dl_dh1[h1==0] = 0

    grads["b1"] = dl_dh1.sum(dim=0)
    grads["W1"] = (torch.mm(dl_dh1.t(), X)).t() + 2*reg*W1
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
      ...
      for lr_decay in learning_rate_decays:
        for hs in hidden_sizes:
          for reg in regularization_strengths:
            print("Training with lr= %.4f, lr_decay= %.2f, reg= %.4f, hs= %d" %(lr, lr_decay, reg, hs), end=" ")
            ...
            print("train_acc= %.4f, val_acc= %.4f" %(stats["train_acc_history"][-1], stats["val_acc_history"][-1]))
            if stats["val_acc_history"][-1] > best_val_acc:
              best_net = net
              best_stat = stats
              best_val_acc = stats["val_acc_history"][-1]
    print("best validation accuracy achieved: %.4f" % best_val_acc)
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
